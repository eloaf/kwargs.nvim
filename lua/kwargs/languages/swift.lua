...
-- Tree-sitter query for Swift function calls with arguments
...
local function get_call_nodes()
    ...
    local query = vim.treesitter.query.parse("swift", ts_query_string)
...
local function get_call_values(call_node)
    ...
    for child_node, _ in argument_list_node:iter_children() do
        ...
        -- In Swift, labeled arguments use label: value syntax
        if child_node:type() == "value_argument" then
            local label_node = child_node:child(0)
            local value_node = child_node:child(1)
            
            if label_node and value_node and label_node:type() == "simple_identifier" then
            ...
            elseif value_node then
                -- Positional argument without label
                local value_text = vim.treesitter.get_node_text(value_node, 0)
                entry.value = value_text
            end
        end
