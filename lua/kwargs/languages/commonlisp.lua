...
-- Tree-sitter query for Common Lisp function calls with arguments
...
local function get_call_nodes()
    ...
    local query = vim.treesitter.query.parse("commonlisp", ts_query_string)
...
local function get_call_values(call_node)
    if call_node:type() ~= "sexp" then
        error("Node is not a sexp node")
    end
    ...
    for child_node in call_node:iter_children() do
        ...
        -- In Common Lisp, keyword arguments use :key value syntax
        if child_node:type() == "symbol" and string.sub(text, 1, 1) == ":" then
            -- This is a keyword symbol, the next node should be its value
            entry.name = string.sub(text, 2)  -- Remove the leading colon
        end
    ...
    -- Now pair up keywords with their values
    local paired_result = {}
    local i = 1
    while i <= #result do
        if result[i].name ~= nil and i + 1 <= #result then
            -- This is a keyword with a following value
            paired_result[#paired_result + 1] = {
                name = result[i].name,
                value = result[i + 1].value,
                node = result[i + 1].node  -- Use the value node for replacement
            }
            i = i + 2
        else
...
local function get_params_from_call_node(call_node)
    -- Extract the position from call_node
    local start_row, start_col = call_node:start()
