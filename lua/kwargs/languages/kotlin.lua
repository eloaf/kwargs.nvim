...
-- Tree-sitter query for Kotlin function calls with arguments
...
local function get_call_nodes()
    ...
    local query = vim.treesitter.query.parse("kotlin", ts_query_string)
...
local function get_call_values(call_node)
    if call_node:type() ~= "call_expression" then
        error("Node is not a call expression node")
    end

    local call_suffix_node = utils.find_first_node_of_type_bfs(call_node, "call_suffix")
    if call_suffix_node == nil then
        error("No call suffix node found")
    end

    local argument_list_node = utils.find_first_node_of_type_bfs(call_suffix_node, "value_arguments")
    ...
    for child_node, _ in argument_list_node:iter_children() do
        ...
        -- In Kotlin, named arguments use name = value syntax
        if child_node:type() == "value_argument" then
            local identifier_node = child_node:child(0)
            local eq_node = child_node:child(1)
            local value_node = child_node:child(2)
            
            if identifier_node and eq_node and value_node and 
               identifier_node:type() == "simple_identifier" and 
               eq_node:type() == "eq" then
                local identifier_text = vim.treesitter.get_node_text(identifier_node, 0)
                local value_text = vim.treesitter.get_node_text(value_node, 0)
                entry.name = identifier_text
