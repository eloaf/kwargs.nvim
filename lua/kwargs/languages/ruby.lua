...
-- Tree-sitter query for Ruby method calls with arguments
...
local function get_call_nodes()
    ...
    local query = vim.treesitter.query.parse("ruby", ts_query_string)
...
local function get_call_values(call_node)
    if call_node:type() ~= "method_call" then
        error("Node is not a method call node")
    end
    ...
    for child_node, _ in argument_list_node:iter_children() do
        ...
        -- In Ruby, keyword arguments use => or : syntax
        -- e.g., key: value or :key => value
        if child_node:type() == "pair" then
            -- Handle key: value syntax
            local key_node = child_node:child(0)
            local value_node = child_node:child(1)
            if key_node and value_node then
                ...
                -- Remove the trailing colon from key if present
                if string.sub(key_text, -1) == ":" then
                    key_text = string.sub(key_text, 1, -2)
                end
                entry.name = key_text
                entry.value = value_text
            end
        elseif child_node:type() == "identifier" and string.sub(text, -1) == ":" then
            -- Handle key: value where we only have the key part
            entry.name = string.sub(text, 1, -2)  -- Remove trailing colon
        end
...
--- Retrieves a method's signature and reformats it into a table of tables.
--- @param call_node TSNode: The node representing the method call.
--- @return table: A table containing the method's arguments with their details.
...
--- Processes a method call node and aligns the call values with the method signature.
-- @param call_node The AST node representing the method call.
...
local function process_call_code(call_node)
    ...
    for j, call_value in ipairs(positional_call_values) do
        aligned[#aligned + 1] = {
            ...
            positional_only = false,  -- Ruby doesn't have positional-only args like Python
